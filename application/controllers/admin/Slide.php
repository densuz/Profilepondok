...
class Slide extends CI_Controller{
	...
	function simpan_slide(){
				$config['upload_path'] = './theme/images/foto_slide/'; //path folder
	            ...
	            if(!empty($_FILES['filefoto']['name']))
	            {
	                if ($this->upload->do_upload('filefoto'))
	                {
	                        ...
	                        $config['source_image']='./theme/images/foto_slide/'.$gbr['file_name'];
	                        $config['create_thumb']= FALSE;
	                        $config['maintain_ratio']= FALSE;
	                        $config['quality']= '60%';
	                        $config['width']= 1800;
	                        $config['height']= 700;
	                        $config['new_image']= './theme/images/foto_slide/'.$gbr['file_name'];
							...
							$this->m_slide->simpan_slide($slide_gambar,$slide_judul,$slide_caption);
	...
	function update_slide(){
	            ...
	            if(!empty($_FILES['filefoto']['name']))
	            {
	                if ($this->upload->do_upload('filefoto'))
	                {
							...
							$path='./theme/images/foto_slide/'.$images;
							unlink($path);
	...
	function hapus_slide(){
		...
		$path='./theme/images/foto_slide/'.$gambar;
		unlink($path);
