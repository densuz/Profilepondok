...
class Testimoni extends CI_Controller{
	...
	function simpan_testimoni(){
				$config['upload_path'] = './theme/images/foto_testi/'; //path folder
	            ...
	            if(!empty($_FILES['filefoto']['name']))
	            {
	                if ($this->upload->do_upload('filefoto'))
	                {
	                        ...
	                        $config['source_image']='./theme/images/foto_testi/'.$gbr['file_name'];
	                        $config['create_thumb']= FALSE;
	                        $config['maintain_ratio']= FALSE;
	                        $config['quality']= '60%';
	                        $config['width']= 500;
	                        $config['height']= 400;
	                        $config['new_image']= './theme/images/foto_testi/'.$gbr['file_name'];
	...
	function update_testimoni(){
	            ...
	            if(!empty($_FILES['filefoto']['name']))
	            {
	                if ($this->upload->do_upload('filefoto'))
	                {
							...
							$path='./theme/images/foto_testi/'.$images;
							unlink($path);
	...
	function hapus_testimoni(){
		...
		$path='./theme/images/foto_testi/'.$gambar;
		unlink($path);
